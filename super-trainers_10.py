...
def filename_csv(clf):
    if clf.__class__.__name__== 'SVC':
        if clf.__class__.get_params(clf)['kernel'] == 'poly':
            return 'Supertrainers_10_' + str(clf.__class__.__name__) + '_kernel=' + str(clf.__class__.get_params(clf)['kernel']) + '_deg=' + str(clf.__class__.get_params(clf)['degree']) + '_mean=' + str(mean_acc)+ '_var=' + str(var_acc) + '.csv'
        else:
            return 'Supertrainers_10_'+ str(clf.__class__.__name__)+ '_kernel='+ str(clf.__class__.get_params(clf)['kernel'])+ '_mean=' +str(mean_acc)+ '_var=' + str(var_acc)  +'.csv'
    if clf.__class__.__name__ == 'KNeighborsClassifier':
        return 'Supertrainers_10_'+ str(clf.__class__.__name__)+ '_k=' +str(clf.__class__.get_params(clf)['n_neighbors']) + '_mean=' + str(mean_acc)+ '_var=' + str(var_acc)  + '.csv'
...
for clf in list_classifier:
    for i in range(500):
        for j in range(10):
            ...
            rd_choice= random.choices(keylist,k=10) #choose k random (image,key number)
        ...
        # adding (accuracy result, i(index of MNIST/10 data sets))
    ...
    Bestresults=acc_results[:1]
    print(Bestresults)
    acc_results.sort(key=takeFirst, reverse=False)
    worsttrainers=acc_results[:1]
    print(worsttrainers)
    print('next')

    Supertrainers=[]
    for image_i_tuple in Bestresults: #looking at the best MNIST/10 data sets
        for j in range(10):
            for size_DS in range(10):
                Supertrainers+=[SortedListofDigits[j][indexes_list[j][image_i_tuple[1]*10+size_DS]]]
