...
def filename_csv(clf):
    if clf.__class__.__name__== 'SVC':
        if clf.__class__.get_params(clf)['kernel'] == 'poly':
            return 'Supertrainers_' + str(clf.__class__.__name__) + '_kernel=' + str(clf.__class__.get_params(clf)['kernel']) + '_deg=' + str(clf.__class__.get_params(clf)['degree']) + '_mean=' + str(mean_acc)+ '_var=' + str(var_acc) + '.csv'
        else:
            return 'Supertrainers_'+ str(clf.__class__.__name__)+ '_kernel='+ str(clf.__class__.get_params(clf)['kernel'])+ '_mean=' +str(mean_acc)+ '_var=' + str(var_acc)  +'.csv'
    if clf.__class__.__name__ == 'KNeighborsClassifier':
        return 'Supertrainers_'+ str(clf.__class__.__name__)+ '_k=' +str(clf.__class__.get_params(clf)['n_neighbors']) + '_mean=' + str(mean_acc)+ '_var=' + str(var_acc)  + '.csv'
...
for clf in list_classifier:
    for i in range(2000):
        for j in range(10):
            ...
            rd_choice= random.choice(keylist) #choose random (image,key number)
            indexes_list[j].append(rd_choice[1]) #store image key
            SortedTrainingExamples += [rd_choice[0]] #add image to training examples
            SortedTrainingLabels+=[j]
    ...
    for image_i_tuple in Bestresults: #looking at the best MNIST/1 data sets
        for j in range(10):
            Supertrainers+=[SortedListofDigits[j][indexes_list[j][image_i_tuple[1]]]]
